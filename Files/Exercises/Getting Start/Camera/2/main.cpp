...
int main(int argc, const char * argv[]) {
...
#ifdef __APPLE__
  string path = "/Camera/2/";
#else
  string path = "\\Camera\\2\\";
#endif

  ShaderReader shader(Settings.CCExercisesPath(path + "EX_GS_C_2_Vertex.shader").c_str(), Settings.CCExercisesPath(path + "EX_GS_C_2_Fragment.shader").c_str());
  ...
  GLint texLoc[2];
  for (int i = 0; i < 2; i++) {
    texLoc[i] = glGetUniformLocation(shader.GetProgram(), names[i]);
  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      modelMat = glm::rotate(modelMat, currentTime * -10.0f, glm::vec3(0.5f, 1.0f, -0.5f));
